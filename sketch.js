...
var backgroundImg;
var manStand, manRun, manImage, manAnimation , manWon , manWonImg ;
...
function draw() {
  ...
  textSize(20);
  fill("black")
  text("Use SPACEBAR TO FIRE",50 , 50);

  text("Use right arrow to move the player forward", 350 , 50);

  text("Make zoom as 90% for the best experiance",800 , 50);

  text("Reach the plane by moving foreward", 1250 , 50)
  ...
  if (keyWentUp(RIGHT_ARROW)) {
    ...
    if(bulletGroup.isTouching(enemy40)){
      enemy40.addImage("enemy",spaceShipImg)
      enemy40.scale=0.8;
      //comment the below line if you want to keep moving 
      enemy40.velocityX=0
      bulletGroup.destroyEach();;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 if(enemy.isTouching(manStand) ) {
      ...
      manStand.destroy();
      bulletGroup.destroyEach();
      gameOver = createSprite(800 , 500 , 20 , 20);
      gameOver.addImage(gameOverImg);
      gameOver.scale = 3 ;

      reload = createSprite(500 , 50 , 20 , 20);
   ...
   if(enemy5.isTouching(manStand) ) {
    ...
    if(enemy20.isTouching(manStand) ) {
      ...
      manStand.destroy();
      bulletGroup.destroyEach();
      gameOver = createSprite(800 , 350 , 20 , 20);
      gameOver.addImage(gameOverImg);
      gameOver.scale = 3 ;
    ...
    if(enemy40.isTouching(manStand) ) {
     ...
     background(stadumImg) ;

     enemy40.visible = 0 ;
     manStand.destroy();
   keyWentDown(RIGHT_ARROW) = false ;
